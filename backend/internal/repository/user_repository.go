...
type UserRepository interface {
	...
	FindActiveByDisplayNames(names []string) ([]model.User, error)
...
func (r *userRepository) List(username string, status *int8, roles []string, page, pageSize int) ([]model.User, int64, error) {
	...
	if err := q.Order("users.id DESC").Find(&items).Error; err != nil {
		return nil, 0, err
	}
...
func (r *userRepository) Exists(id uint64) (bool, error) {
	...
	var cnt int64
	if err := model.DB.Model(&model.User{}).Where("id = ?", id).Count(&cnt).Error; err != nil {
		return false, err
	}
	return cnt > 0, nil
}
...
func (r *userRepository) FindActiveByDisplayNames(names []string) ([]model.User, error) {
	if len(names) == 0 {
		return []model.User{}, nil
	}
	...
	var users []model.User
	tx := model.DB.Session(&gorm.Session{NewDB: true}).
		Table("users").
		Select("users.id, users.username, users.alias, users.email, users.phone, users.status, users.last_login_at, users.created_at, users.updated_at").
		Where("users.status = 1").
		Where("COALESCE(NULLIF(users.alias, ''), users.username) IN ?", uniq)
	if err := tx.Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
...
func (r *userRepository) UsernameExists(username string) (bool, error) {
	...
	var cnt int64
	if err := model.DB.Model(&model.User{}).Where("username = ?", username).Count(&cnt).Error; err != nil {
		return false, err
	}
	return cnt > 0, nil
}
