...
func (r *syncRulesRepository) ListDistinctCustomerRegions() ([]string, error) {
	return (&ratesRepository{}).ListDistinctCustomerRegions()
}

func (r *syncRulesRepository) ListDistinctCustomerCPs() ([]string, error) {
...
func (r *syncRulesRepository) List(filter map[string]interface{}, limit, offset int) ([]model.RateCustomerSyncRule, int64, error) {
	...
	if total == 0 {
		return []model.RateCustomerSyncRule{}, 0, nil
	}
	if limit > 0 {
		q = q.Limit(limit)
	}
