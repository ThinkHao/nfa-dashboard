    ...
    for _, rule := range rules {
        ...
        if len(extraSet) > 0 {
            for k, v := range extraSet {
                if _, exists := setMap[k]; !exists { setMap[k] = v }
            }
        }
        ...
        for k := range setMap { setKeys = append(setKeys, k) }
        ...
        for _, region := range regionList {
            for _, cp := range cpList {
                ...
                if region != "*" { schoolFilter["region"] = region }
                if cp != "*" { schoolFilter["cp"] = cp }
                const pageSize = 500
                page := 1
                for {
                    ...
                    for i := range schools {
                        ...
                        if updated {
                            ...
                            for k := range fieldUpdates { updKeys = append(updKeys, k) }
                            ...
                            if existed {
                                if fieldUpdates == nil { fieldUpdates = map[string]interface{}{} }
                                fieldUpdates["last_sync_time"] = now
                                fieldUpdates["last_sync_rule_id"] = rule.ID
                                if err := s.ratesRepo.UpdateCustomerByID(rc.ID, fieldUpdates); err != nil { return totalAffected, err }
                            } else {
                                // 新建记录：将同步信息写入结构体并 Upsert
                                rid := rule.ID
                                rc.LastSyncTime = &now
                                rc.LastSyncRuleID = &rid
                                if err := s.ratesRepo.UpsertCustomerRate(&rc); err != nil { return totalAffected, err }
                            }
                            totalAffected++
                        }
                    }
...
func (s *ratesSyncService) applyRuleToCustomer(rc *model.RateCustomer, rule model.RateCustomerSyncRule, whitelist []string, setMap map[string]interface{}) (bool, map[string]interface{}, error) {
    ...
    for k, v := range setMap {
        ...
        if _, isTop := topFields[k]; isTop {
            ...
            case int64:
                x := float64(t); f = &x
            case json.Number:
                if fv, err := t.Float64(); err == nil { f = &fv }
            case string:
                // 简单尝试解析
                if num, err := json.Number(t).Float64(); err == nil { f = &num }
            }
            // 计算覆盖策略
            var curPtr **float64
            switch k {
    ...
    if !changed { return false, nil, nil }
...
func parseActionsSet(data []byte) (map[string]interface{}, error) {
    ...
    if t, ok := obj["type"].(string); ok && t == "template" {
        if vals, ok := obj["values"].(map[string]interface{}); ok { return vals, nil }
...
func parseFieldsToUpdateExtra(data []byte) (map[string]interface{}, error) {
    if len(data) == 0 { return map[string]interface{}{}, nil }
    var obj map[string]interface{}
    if err := json.Unmarshal(data, &obj); err != nil { return map[string]interface{}{}, nil }
    raw, ok := obj["extra"]
    if !ok || raw == nil { return map[string]interface{}{}, nil }
    m, ok := raw.(map[string]interface{})
    if !ok { return map[string]interface{}{}, nil }
