	...
	for _, rule := range rules {
		...
		for _, region := range regionList {
			for _, cp := range cpList {
				...
				existingKeys, err := s.ratesRepo.ListCustomerRateKeys(schoolFilter)
				if err != nil {
				...
				for {
					...
					for i := range schools {
						sch := schools[i]
						key := syncCustomerRateKey(sch.Region, sch.CP, sch.SchoolName)
						...
						if updated {
							...
							log.Printf("[rates-sync] apply insert: region=%s cp=%s school=%s keys=%v", rc.Region, rc.CP, derefString(rc.SchoolName), updKeys)
							rid := rule.ID
							rc.LastSyncTime = &now
							rc.LastSyncRuleID = &rid
							inserted, err := s.ratesRepo.CreateCustomerRateIfMissing(&rc)
							if err != nil {
								return totalAffected, err
							}
	...
	if rule.ConditionExpr != nil {
		...
		if expr != "" {
			...
			if rc.CustomerFee != nil {
				ctx["customer_fee"] = strconv.FormatFloat(*rc.CustomerFee, 'f', -1, 64)
			}
			if rc.NetworkLineFee != nil {
				ctx["network_line_fee"] = strconv.FormatFloat(*rc.NetworkLineFee, 'f', -1, 64)
			}
			if rc.GeneralFee != nil {
				ctx["general_fee"] = strconv.FormatFloat(*rc.GeneralFee, 'f', -1, 64)
			}
			if rc.ChannelRate != nil {
				ctx["channel_rate"] = strconv.FormatFloat(*rc.ChannelRate, 'f', -1, 64)
			}
...
func syncCustomerRateKey(region, cp, schoolName string) string {
