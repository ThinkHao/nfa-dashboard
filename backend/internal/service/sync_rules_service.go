...
type syncRulesService struct {
	repo repository.SyncRulesRepository
}
...
func (s *syncRulesService) ListOptions() ([]string, []string, error) {
...
func (s *syncRulesService) Create(rule *model.RateCustomerSyncRule) (*model.RateCustomerSyncRule, error) {
	...
	if !isValidOverwriteStrategy(rule.OverwriteStrategy) {
		return nil, NewBadRequest("invalid overwrite_strategy")
	}
	// JSON 字段校验
	if err := validateStringArrayJSON(rule.ScopeRegion, true); err != nil {
		return nil, err
	}
	if err := validateStringArrayJSON(rule.ScopeCP, true); err != nil {
		return nil, err
	}
	if err := validateStringArrayJSON(rule.FieldsToUpdate, true); err != nil {
		return nil, err
	}
	if len(rule.Actions) == 0 {
		return nil, NewBadRequest("actions is required and must be valid JSON")
	}
	if !json.Valid(rule.Actions) {
		return nil, NewBadRequest("actions must be valid JSON")
	}
...
func (s *syncRulesService) Update(id uint64, updates map[string]interface{}) error {
	...
	if v, ok := updates["overwrite_strategy"]; ok {
		if v == nil {
			return NewBadRequest("overwrite_strategy cannot be null")
		}
		srt := strings.ToLower(strings.TrimSpace(v.(string)))
		if !isValidOverwriteStrategy(srt) {
			return NewBadRequest("invalid overwrite_strategy")
		}
		updates["overwrite_strategy"] = srt
	}
	if v, ok := updates["scope_region"]; ok {
		if v == nil {
			return NewBadRequest("scope_region cannot be null")
		}
		if err := validateStringArrayJSONInterface(v, true); err != nil {
			return err
		}
	}
	if v, ok := updates["scope_cp"]; ok {
		if v == nil {
			return NewBadRequest("scope_cp cannot be null")
		}
		if err := validateStringArrayJSONInterface(v, true); err != nil {
			return err
		}
	...
	if v, ok := updates["fields_to_update"]; ok {
		if v == nil {
			return NewBadRequest("fields_to_update cannot be null")
		}
		if err := validateStringArrayJSONInterface(v, true); err != nil {
			return err
		}
		...
		if arr, ok2 := mustParseStringArrayInterface(v); ok2 {
			for _, fk := range arr {
				if !isValidFieldKey(fk) {
					return NewBadRequest("fields_to_update contains invalid field_key")
				}
			}
		}
	}
	if v, ok := updates["actions"]; ok {
		if v == nil {
			return NewBadRequest("actions cannot be null")
		}
		// 接受任意合法 JSON，但不能为空对象/数组
		var any interface{}
		bs, ok2 := toJSONBytes(v)
		if !ok2 || !json.Valid(bs) {
			return NewBadRequest("actions must be valid JSON")
		}
		...
		// 简单非空检查
		if isEmptyJSON(any) {
			return NewBadRequest("actions cannot be empty")
		}
	}
...
func validateStringArrayJSON(data []byte, allowEmpty bool) error {
	...
	if err := json.Unmarshal(data, &arr); err != nil {
		return NewBadRequest("must be JSON array of strings")
	}
	if !allowEmpty && len(arr) == 0 {
		return NewBadRequest("array cannot be empty")
	}
	return nil
}
...
func validateStringArrayJSONInterface(v interface{}, allowEmpty bool) error {
	bs, ok := toJSONBytes(v)
	if !ok {
		return NewBadRequest("must be valid JSON")
	}
