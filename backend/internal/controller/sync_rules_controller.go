...
func NewSyncRulesController(svc service.SyncRulesService) *SyncRulesController {
	return &SyncRulesController{svc: svc}
}

func (ctl *SyncRulesController) ListOptions(c *gin.Context) {
...
func (ctl *SyncRulesController) Create(c *gin.Context) {
	...
	if req.FieldsToUpdate != nil {
		rule.FieldsToUpdate = datatypes.JSON(*req.FieldsToUpdate)
	}
...
func (ctl *SyncRulesController) Update(c *gin.Context) {
	...
	if req.ScopeRegion != nil {
		b := datatypes.JSON(*req.ScopeRegion)
		updates["scope_region"] = b
	}
	if req.ScopeCP != nil {
		b := datatypes.JSON(*req.ScopeCP)
		updates["scope_cp"] = b
	}
	...
	if req.FieldsToUpdate != nil {
		b := datatypes.JSON(*req.FieldsToUpdate)
		updates["fields_to_update"] = b
	}
	if req.OverwriteStrategy != nil {
		updates["overwrite_strategy"] = *req.OverwriteStrategy
	}
	if req.Actions != nil {
		b := datatypes.JSON(*req.Actions)
		updates["actions"] = b
	}
